...
Landscape patterns and their quantification have received considerable attention since the early 1980s. The quantification of landscape patterns came from the need to objectively describe landscapes that humans assess subjectively as, for example, "clumpy", "dispersed", "random", "diverse", "fragmented", or "connected" (@gergel2017). Quantification of pattern is fundamental to many of the relationships we seek to understand in landscape ecology, because they emphasize the interactions among spatial patterns and ecological processes. Therefore, a basic familiarity with the most commonly used ways to quantify these patterns is extremely important (@gergel2017). In general, we refer to these as **landscape metrics**. Landscape metrics allow us to objectively describe landscape patterns at various spatial scales based on certain observable features, which enable analyses such as:

- Assessment of landscape pattern changes over time
- Making future predictions regarding landscape pattern changes
- Determining if two landscapes differ based on their patterns
- Evaluating alternative land management strategies in terms of the landscape patterns that may result
- Determining if a certain pattern is characteristic of a specific disturbance

In particular, in this lab we will take advantage of both R- and python-based libraries to objectively quantify landscape patterns to answer specific research questions. Both these libraries are inspired by _FRAGSTATS_ (@mcgarigal1995). In more detail, we will explore the use of landscape metrics to compare spatial patterns created by 5 different fires (@fig-fires). Furthermore, we will focus on comparing spatial patterns of fires of different severity to see how they produce different spatial responses on the landscape.

We mentioned that landscape metrics can be calculated at different spatial scales. In general, we recognize three distinct levels of organization: **patch**, **class**, and **landscape**. Patch-level metrics refer to metrics calculated over a single contiguous unit, for example a single group of neighboring pixels belonging to the same group (e.g. coniferous land cover). Class-level metrics represent metrics that are averaged across all patches belonging to the same broader category. For instance, calculating the class-level "patch area" of a landscape characterized by 10 coniferous patches, means calculating the individual patch area of each coniferous patch and averaging their values. Finally, metrics calculated at the landscape level are summary aggregates of patch- or class-level metrics regardless of the individual classes.

Another important concept to understand is **neighboring**. When working with landscape metrics, individual pixels and their neighbors are both important. Take for example @fig-nn, we are working with the central pixel marked with a cross. How do we define a uniform patch or region? Do we consider only the pixels north-south and west-east, or do we also consider the diagonals? This is important because by changing the neighboring rule we change how patches, and therefore metrics, are interpreted and calculated. For example, using a 4-neighboring rule, the patch would have an area of 5, whereas using an 8-neighboring rule it would have an area of 9!

![Illustration of 2 neighboring rules: (left) 4-neighbors, (right) 8-neighbors.](data/neighbors.png){#fig-nn}

Note that one core assumption that these and other methods take is that the input data source is a 2-dimensional (2D) **categorical raster**. A categorical raster is a raster product that describes homogeneous landscape features with integer numbers (@fig-lands). However, it is often the case that you'd need to convert your **continuous raster** product into a categorical raster. The simplest way to do it is to define a threshold value where pixels above or below such threshold are assigned to one class or another. Conversion is a crucial step in the calculation of landscape patterns, because the selection of the threshold value would affect the quantification of the landscape patterns of interest. We will talk more about thresholding later on.
...
# Getting familiar with FRAGSTATS

Historically, landscape patterns were quantified using a standalone program called FRAGSTATS (@mcgarigal1995). FRAGSTATS provides a visual, code-free way to explore the core concepts of patch, class, and landscape levels without the need to debug scripts. This allows you to focus on understanding the metrics and their ecological meaning before moving on to more complex coding workflows. In this section, we will follow a short, simple walkthrough to see how FRAGSTATS operates. However, the core of the lab is tailored towards more automated solutions developed in R and Python, which are generally preferred.

- Step 0: Download and install the stable version of FRAGSTATS [here](https://FRAGSTATS.org/index.php/downloads)
- Step 1: Open FRAGSTATS. Create a new project by clicking “New” on the top ribbon. You can also select “File” -\> “New” on the top tab.
- Step 2: Add layer by selecting “Add layer…”. A new dialogue box appears to select the data type. For this lab, you should choose GeoTIFF grid (.tif). For "Dataset name", click on the “…” button and choose the desired .tif file (e.g., fire_1_high.tif). Your layer should appear in the “Layers” box, under the “Batch management” screen.

![Add .tif file to input layer in FRAGSTATS](data/fragstat_s2.png){#fig-add-file}

- Step 3: Before running any analysis, we should adjust some parameters in “Analysis parameters", next to the “Input layers” tab. Here, you can choose the neighborhood rule (in this lab, it is 8 cell neighborhood rule). The sampling strategy is “No sampling”. The level of the metrics is “Patch” (feel free to explore other metrics)

![Adjust the parameters](data/fragstat_s3.png){#fig-adj}

- Step 4: On the right side of the screen, you can switch between Patch metrics, Class metrics, and Landscape metrics. Within each of these tabs, different groups of metrics are available, such as Area–Edge, Core area, Shape, and Aggregation. For this lab exercise, go to the Patch metrics tab, open the Area–Edge sub-tab, and select Patch Area (AREA) for analysis.

![Selecting the landscape metrics](data/fragstat_s4.png){#fig-sel}

- Step 5: Click “Run” and check the results in the “Results” tab

The FRAGSTATS output shows patch-level results for "cls_1" and "cls_2" (representing “unburned” and “burned”) in your landscape. These values help determine whether each class is dominated by one large patch or many small, scattered patches, which can reveal important ecological patterns such as extensive burn areas or unburned refugia.

In the next section, we will continue with landscape metric calculations in R and Python.

# Exploring basic landscape metrics

Here, we will calculate landscape metrics from a categorical fire severity map using both R- and python-based solutions. In R we are using the `landscapemetrics` library (@hesselbarth2019), in python `pylandstats` (@bosch2019). Our objective is to describe landscape patterns created by 5 fire events of varying severity in Greater Yellowstone. Let's start by importing libraries and data. In the following chunks, only the R code is evaluated because it is more user-friendly than the python alternative as of today.
...
flagged <- lapply(fires, function(f) {
  r <- rast(f)
  NAflag(r) <- 99
  crs(r) <- 'epsg:32612'  # manually assign CRS because originally missing
...
# metrics of interest
multiple_metrics <- calculate_lsm(
  landscape = flagged[[1]],
...
To this list of metrics, we ask you to provide 5 additional ones to better explore the spatial pattern of fire in a way we are not able to using the metrics we proposed. Calculate these metrics for all fire rasters you have. This can be accomplished using a simple `for loop` in Python or by feeding a list of rasters or a raster stack to the `calculate_lsm` function in R. Use an 8-neighboring rule for your metrics if applicable. You can conduct the analysis in the language you are the most comfortable with. Your coding skills are not subject to any grading in our labs.
...
**_Question 1: Provide the rationale for each new metric that you selected for your analysis using a bullet point for each._**

**_Question 2: Why may metric redundancy be an issue in the analysis of landscape patterns?_**
...
$$ NBR = \frac{(NIR - SWIR2)}{(NIR + SWIR2)} ∈ [-1, 1] $$ $$ dNBR = (PreFireNBR - PostFireNBR) ∈ [-2, 2] $$ {#eq-dnbr}

Where NIR is the near infrared wavelength and SWIR2 is the short-wave infrared wavelength.

If you take a closer look at the metrics we computed earlier and plot them (@fig-boxes), you would notice that we have 2 threshold values, specifically for dNBR. A low threshold indicates that fires are characterized early, or when they are less severe. A higher threshold only characterizes high severity fires. A low threshold value results in a higher contagion value, indicating a higher aggregation level than high severity fires (mostly because we include lower severity fires as well), as well as less complex shapes as indicated by the lower edge density values. Similarly, class-level metrics indicate larger and more numerous patches on average for the burned class. Inverse trends are expected for the unburned area class.
...
**_Question 3: Briefly describe using bullet points 3 main differences in spatial patterns that were characterized by the low vs. high dNBR thresholds. Refer to your 5 new landscape metrics in the answer._**

**_Question 4: Briefly describe how a change in neighboring rule may affect the quantitative estimates of burn patterns. Did all metrics change in the same way?_**

**_Question 5: How does the choice of the landscape metrics and threshold affect the ecological interpretation of the burned patterns on the landscape?_**
