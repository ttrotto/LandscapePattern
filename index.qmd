...
ggplot() +
  ...
  scale_fill_manual(values = c('unburned' = '#c1be7a', 'burned' = '#9f0a1a', 'modata' = '#ababab'))
```
...
metrics_class <- metrics_class %>%
     mutate(layer = recode(layer,
         ...
         "10" = "fire_5_low"))
metrics_landscape <- metrics_landscape %>% 
  mutate(across(layer, metric), as.factor) %>%
  separate(col=layer, into = c("fire", "landscape", "threshold"), sep="_") %>%
  mutate(rule = "8")
metrics_class <- metrics_class %>% 
  mutate(across(layer, class, metric), as.factor) %>%
  separate(col=layer, into = c("fire", "landscape", "threshold"), sep="_") %>%
  mutate(rule = "8")
