...
Although R and Python are the primary tools used in this lab, FRAGSTATS will be used as a supplementary tool to introduce landscape metrics. It provides a visual, code-free way to explore the core concepts of patch, class, and landscape levels without the need to debug scripts. This allows you to focus on understanding the metrics and their ecological meaning before moving on to more complex coding workflows. In this section, you will follow a short, simple walkthrough to see how FRAGSTATS operates. By the end, you will produce an output that you can compare with the results obtained from R and Python.

-   Step 0: Download and install FRAGSTAT via this link: https://fragstats.org/index.php/downloads (Recommended version: frg4.2.681\[x64\])

-   Step 1: Open FRAGSTAT. Then, create a new project by clicking “New” on the ribbon. You can also select “File” -\> “New” on the tab.

-   Step 2: Add layer by selecting “Add layer…”. A new dialogue box appears and it asks you to select the data type. For this lab, you should choose GeoTIFF grid (.tif). For Dataset name, click on the “…” button and choose the desired .tif file (e.g., fire_1_high.tif)

![Add .tif file to input layer in Fragstats](data/fragstat_s2.png) Your layer should appear in the “Layers” box, under the “Batch management” screen

-   Step 3: Before performing any analysis, we should do some settings by adjusting parameters in “Analysis parameters’, which is located next to the “Input layers” tab. Here, you can choose the neighborhood rule (in this lab, it is 8 cell neighborhood rule). The sampling strategy is “No sampling”. The level of the metrics is “Patch” (feel free to explore other metrics)

![Adjust the parameters](data/fragstat_s3.png)
...
![Selecting the landscape metrics](data/fragstat_s4.png)
...
$$ NBR = \frac{(NIR - SWIR)}{(NIR + SWIR)} ∈ (0, 1) $$ $$ dNBR = (PreFireNBR - PostFireNBR) ∈ (0, 1) $$ {#eq-dnbr}
